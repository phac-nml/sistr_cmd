...
from sistr_cmd.misc.reduce_to_centroid_alleles import run_allele_reduction
from sistr_cmd.sistr_cmd import genome_name_from_fasta_path
...
from sistr_cmd.serovar_prediction import SerovarPredictor, overall_serovar_call
from sistr_cmd.cgmlst import CGMLST_PROFILES_PATH, cgmlst_serovar_prediction, hash_allele_seq, CGMLST_FULL_FASTA_PATH
from sistr_cmd.serovar_prediction import GENOMES_TO_SEROVAR_PATH, GENOMES_TO_SPP_PATH, SEROVAR_TABLE_PATH
from sistr_cmd.mash import MASH_SKETCH_FILE
...
def merge_sketches(outdir, sketch_paths):
    ...
    """
    merge_sketch_path = os.path.join(outdir, 'sistr_cmd.msh')
...
def run_sistr(input_fasta, tmp_dir):
    ...
    try:
        ...
        cgmlst_prediction, cgmlst_results = cgmlst_serovar_prediction(blast_runner)
...
def write_cgmlst_fasta(outdir, cgmlst_results):
    ...
    with open(new_cgmlst_fasta_path, 'w') as fout:
        for marker in sorted(marker_allele_seqs.keys()):
            ...
            for seq in seqs:
                fout.write('>{}|{}\n{}\n'.format(marker, hash_allele_seq(seq), seq))
